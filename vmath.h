...
{
	float top = n * static_cast<float>(tan(radians(0.5 * static_cast<double>(fovy)))); // bottom = -top
...
static inline vec3 lerp(vec3 start, vec3 end, float percent)
{
	return (start + percent * (end - start));
}

static inline vec3 catmullRom(vec3 p0, vec3 p1, vec3 p2, vec3 p3, float t)
{
	float t2 = t * t;
	float t3 = t2 * t;
	return 0.5f * ((2.0f * p1) +
